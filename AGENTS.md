...
- Workers demo: `mise run demo:workers:{dev,serve,run,test,load,deploy,smoke-remote}`
...
- Add components with `gsxui add <name>` (run in the demo's directory). Don't hand-copy from upstream.
...
- Ports: workerd 8913 / `go run .` 9913 (Workers demo), 8918 (gsxui demo on workerd), 7777 (gsxui native). 8787 and 8797 are often taken
...
- `demos/workers/` - htmx 4 on Cloudflare Workers demo (`main.go` + `board.go` handlers, `layout`/`home`/`board`/`fragments`/`theme` `.gsx`, `ui/` vendored by gsxui,
  `store_{sql,mem}.go`, `platform_{js,other}.go`, `index.mjs` entry + `room.mjs` Durable Object, `migrations/`, `public/` (vendored htmx),
  `web/gsxui/` (behaviours + CSS entry), `config.capnp` + `workerd/` for local workerd, `cmd/deploy` (Direct Upload + D1 + DO), `cmd/wsload`).
...
- htmx endpoints return HTML fragments rendered from gsx components: `s.render(...)` in `demos/gsxui`, `writeNode(...)` in
  `demos/workers`. No string-built HTML, no `html/template` (it panics under TinyGo).
