...
func TestLocales(t *testing.T) {
	...
	ok := b.waitJS(5*time.Second, `location.pathname === "/ar/formats" && document.documentElement.lang === "ar"`)
	...
	// Boosted nav inside the locale keeps the prefix and the RTL document.
	marker = b.num(`window.__marker = Math.floor(Math.random() * 1e9)`)
	...
	check(t, "/ follows the remembered locale", b.waitJS(5*time.Second, `location.pathname === "/ar/" && document.documentElement.lang === "ar"`))

	// Back to English via the list: / stays English afterwards.
	b.eval(`document.querySelector('#languages a[hreflang="en"]').click()`, nil)
	b.waitJS(5*time.Second, `location.pathname === "/" && document.documentElement.lang === "en"`)
	b.run(chromedp.Navigate(base+"/"), chromedp.WaitReady("main", chromedp.ByQuery))
	check(t, "choosing English sticks", b.waitJS(5*time.Second, `location.pathname === "/" && document.documentElement.lang === "en"`))
