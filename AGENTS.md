...
- The app is the Go module at the repo root (`github.com/joeblew999/go-htmx4`). `demos/gsxui` is a separate module being
  merged into it (plan `.plans/2026-09-14_1037_one-app-template-and-kit.md`).

# Commands

- Test everything: `mise run check` (`test` + `demo:gsxui:test` + `demo:gsxui:workers:smoke`). It must pass
...
- gsxui demo: `mise run demo:gsxui:{run,dev,test}` (native, :7777), `mise run demo:gsxui:workers:{dev,serve,smoke,deploy}`
...
- gsx v0.1.0 walks into nested Go modules with the outer `gsx.toml`: at the root, pass dirs (`gsx generate views ui`), and
  `gsx dev` (no path args) fails while `demos/gsxui` exists.
- Add components with `gsxui add <name>` (run at the repo root for the app). Don't hand-copy from upstream.
...
- Ports: workerd 8913 / `go run .` 9913 (the app), 8918 (gsxui demo on workerd), 7777 (gsxui native). 8787 and 8797 are often taken
...
- htmx endpoints return HTML fragments rendered from gsx components: `s.render(...)` in `demos/gsxui`, `writeNode(...)` in
  the app. No string-built HTML, no `html/template` (it panics under TinyGo).
